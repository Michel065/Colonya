...
Hub::Hub(Map* map, TimeManager* tm) : Structure(StructureType::HUB,
...
bool Hub::entrer(Entite* e) {
    std::lock_guard<std::mutex> lock(mtx_entite);
    if (entites_presentes.size() >= capacite_entree) return false;

    if (std::find(entites_presentes.begin(), entites_presentes.end(), e) != entites_presentes.end())
        return false;
...
bool Hub::dormir(Entite* e){
    ...
    int val=5;
    if(carte->il_fait_jour() && e->peux_dormir())e->dormir(val);
    else if(e->peux_dormir()) e->dormir(val*2);
    else return false;
    return true;
}
...
nlohmann::json Hub::get_json() const {
    ...
    // Sauvegarde des noms des entités présentes
    {
        std::lock_guard<std::mutex> lock(mtx_entite);
        j["entites_presentes"] = nlohmann::json::array();
        for (const auto* e : entites_presentes) {
            j["entites_presentes"].push_back(e->get_name()); // nom = identifiant
        }
    }
...
void Hub::from_json(nlohmann::json json) {
    ...
    // Sauvegarde les noms temporairement, les pointeurs seront rebind plus tard
    if (json.contains("entites_presentes")) {
        std::lock_guard<std::mutex> lock(mtx_entite);
        entites_presentes.clear(); // nettoyage préventif

        for (const auto& nom : json["entites_presentes"]) {
            std::string id = nom.get<std::string>();

            // On ne peut pas encore restaurer le Entite*, mais on garde l'info
            // Plus tard on fera : Entite* e = EntiteManager::get_from_nom(id);
            // entites_presentes.push_back(e); ← quand ce système sera prêt
